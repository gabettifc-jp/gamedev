...
# 渡された八つの枠に入らなかったもの（2026-08-21・追加で聞いた）

**枠は、枠に入らないものを落とす。**集め終わったあとに聞いたら、**10件出てきた。**
どれも「大事だが、渡された八分野のどれでもない」もの。**枠のほうが悪かった証拠として残す。**

| # | 何が | どの出どころ | なぜ八つに入らないか |
|---|---|---|---|
| 1 | **遊ばれる前の見た目**（ストアのサムネイル・差別化） | 028／008／038 | 八つは全部「遊んでいる最中の画面」が前提。**231px の一枚として並ぶとき**は要求される性質が違う |
| 2 | **見た目そのものを進行の報酬にする** | 033（Downwell の解禁パレット） | 「どう見せるか」ではなく「**見た目を何に使うか**」 |
| 3 | **絵とルールの一致**（当たり判定と見た目のずれ） | 009／020 | 「読める」ではなく「**読めたとおりに当たる**」。3と6に跨がってどちらにも収まらない |
| 4 | **選んだ見た目が、後から要求する保守** | 031（192枚→288枚）／019／004 | 7は「少ない手数で始める道」。これは「**選んだあとの債務**」 |
| 5 | **作り手自身のぶれ**（同じ人が同じものを描けない） | 030 | 「統一しろ」の理由が「複数人だから」ではなく「**一人でも日によって変わるから**」 |
| 6 | **表示環境の差**（別の機械では別物になる） | 036/037/040／010／011 | 八つは全部「画面の中」。**どの画面で・どれだけ離れて・どういう照明で**が入る場所が無い |
| 7 | **「見づらい」の先にある身体症状** | 023（頭痛・吐き気・めまい）／038 | **見づらいは我慢できるが、吐くのは我慢できない。**原因も別の層（後処理・カメラ挙動） |
| 8 | **色以外の手段で、同じ情報を二重に符号化する** | 024／010 | 3はシルエットと対比の話。**同じ情報を二つの経路で流す**のは別の手 |
| 9 | **音との組み合わせを切ると落ちるもの** | 009（三つの vow が全部「visuals and audio」）／019 | 題が「見た目」なので意図的な除外かもしれないが、**一次資料の側が視聴覚を分けていない** |
| 10 | **「規律を破ってよいのはいつか」** | 004（"intent matters"）／038 | 八つは全部「どうすれば良くなるか」。**破る側の条件**が入る場所が無い |

## 八つの切り方そのものへの指摘（集めた側から）

- **3（読みやすさ）と 5（画面の重み付け）は、実質同じものを別角度から見ている。**
  同じ手段（明度差・彩度差・形の差）で作られているので、**同じ出どころを二回書くことになった**
- **1（値）と 2（パレット）も分けづらい。**
  001 が「value・saturation・hue は独立に扱えない、perceived value は三属性すべてに依存する」と明言している
- **4 は実は二つ。**「ピクセル密度・粒の大きさ」と「字の大きさ」。
  **出どころの層がまったく違う**（ピクセルアート講座 vs アクセシビリティ指針）
- **6 も二つ。**「アニメの枚数と間」と「手応えの演出」。
  後者は**絵を描かずに印象を変える話**で、7 のほうに近い
- **7 も二つ。**「絵を持たない作り」と「絵が描けない人の失敗を封じる規律」
- **8 も二つで、原因が違う。**「安っぽい」＝不統一／「見づらい」＝コントラスト不足。
  同じ枠にしたので、**両方を同時に言われている件を分解できなかった**
